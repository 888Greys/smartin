...
// Run with: node scripts/update-referral-codes.mjs
...
async function updateReferralCodes() {
    ...
    const usersToUpdate = users.filter(u => u.referralCode && u.referralCode.length > 6);
    ...
    for (const user of usersToUpdate) {
        ...
        try {
            ...
            console.log(`✅ ${user.email}: ${user.referralCode.slice(0, 10)}... → ${newCode}`);
            updated++;
        } catch (error) {
    ...
    console.log(`   ⏭️  Skipped: ${users.length - usersToUpdate.length} (already short or empty)`);
}
